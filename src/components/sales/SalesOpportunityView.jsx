...
export default function SalesOpportunityView() {
  ...
  const handleFileUpload = async (e, stage, docType) => {
    ...
    try {
      ...
      if (!url) {
        alert("Please save the opportunity first before uploading files.");
        return;
      }
  ...
  const advanceStage = () => {
    if (canAdvance() && activeStage < 7) {
      ...
      alert("Cannot advance. Please ensure all required approvals/fields (Go/No-Go, Approvals) are set to 'Yes'.");
    }
  };
